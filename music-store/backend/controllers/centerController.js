...
const getAllCourses = async (req, res) => {
    try {
        const [rows] = await db.query(`
            SELECT co.*, cat.category_name,
                COUNT(DISTINCT cl.class_id) AS total_classes
            FROM courses co
            LEFT JOIN categories cat ON co.category_id = cat.category_id
...
const createCourse = async (req, res) => {
    try {
        const { course_name, category_id, level, description, duration_months, sessions_total, tuition_fee } = req.body;
        if (!course_name || !tuition_fee) return res.status(400).json({ message: 'Thiếu thông tin!' });
        const [r] = await db.query(
            `INSERT INTO courses (course_name,category_id,level,description,duration_months,sessions_total,tuition_fee)
             VALUES (?,?,?,?,?,?,?)`,
            [course_name, category_id||null, level||'beginner', description||null, duration_months||3, sessions_total||0, tuition_fee]
        );
...
const updateCourse = async (req, res) => {
    try {
        const fields = ['course_name','category_id','level','description','duration_months','sessions_total','tuition_fee','is_active'];
        const sets = [], vals = [];
...
const getAllClasses = async (req, res) => {
    try {
        ...
        let sql = `
            SELECT cl.*, co.course_name, co.tuition_fee, t.full_name AS teacher_name, t.specialty,
...
const getClassDetail = async (req, res) => {
    try {
        ...
        const [students] = await db.query(`
            ...
            WHERE e.class_id=?
            GROUP BY e.enrollment_id`, [req.params.id]);
...
const createClass = async (req, res) => {
    try {
        ...
        if (!class_name || !course_id || !teacher_id || !start_date) return res.status(400).json({ message: 'Thiếu thông tin!' });
        const [r] = await db.query(
            `INSERT INTO classes (class_name,course_id,teacher_id,max_students,room,schedule_days,schedule_time,start_date,end_date)
             VALUES (?,?,?,?,?,?,?,?,?)`,
            [class_name, course_id, teacher_id, max_students||10, room||null, schedule_days||null, schedule_time||null, start_date, end_date||null]
        );
...
const enrollStudent = async (req, res) => {
    try {
        ...
        const final_amount = cls.tuition_fee - discount;

        const [r] = await db.query(
            `INSERT INTO enrollments (student_id,class_id,tuition_total,discount_amount,final_amount,note)
             VALUES (?,?,?,?,?,?)`,
            [student_id, class_id, cls.tuition_fee, discount, final_amount, note||null]
        );
...
const updateEnrollment = async (req, res) => {
    try {
        ...
        await db.query('UPDATE enrollments SET status=?, note=? WHERE enrollment_id=?', [status, note||null, req.params.id]);
        res.json({ message: 'Cập nhật đăng ký thành công!' });
    } catch(e) { res.status(500).json({ message: 'Lỗi server!', error: e.message }); }
};

// =============================================
// TUITION PAYMENTS (Học phí)
// =============================================
    ...
    try {
        ...
        await db.query(
            `INSERT INTO tuition_payments (enrollment_id,amount,payment_date,payment_method,note,created_by)
             VALUES (?,?,?,?,?,?)`,
            [enrollment_id, amount, payment_date||new Date().toISOString().split('T')[0], payment_method||'cash', note||null, req.user.user_id]
        );
...
const getPaymentsByEnrollment = async (req, res) => {
    try {
        const [rows] = await db.query(
            'SELECT tp.*, u.full_name AS created_by_name FROM tuition_payments tp LEFT JOIN users u ON tp.created_by=u.user_id WHERE tp.enrollment_id=? ORDER BY tp.payment_date DESC',
...
const createSession = async (req, res) => {
    try {
        ...
        if (!class_id || !session_date || !start_time || !end_time) return res.status(400).json({ message: 'Thiếu thông tin!' });
        const [r] = await db.query(
...
const saveAttendance = async (req, res) => {
    try {
        ...
        if (!session_id || !Array.isArray(records)) return res.status(400).json({ message: 'Dữ liệu không hợp lệ!' });
...
const getAttendanceByStudent = async (req, res) => {
    try {
        const [rows] = await db.query(`
            SELECT a.*, cs.session_date, cs.start_time, cs.end_time, cs.topic,
                cl.class_name
            FROM attendance a
            JOIN class_sessions cs ON a.session_id = cs.session_id
            JOIN classes cl ON cs.class_id = cl.class_id
            WHERE a.student_id=?
            ORDER BY cs.session_date DESC`, [req.params.student_id]);
        res.json(rows);
    } catch(e) { res.status(500).json({ message: 'Lỗi server!', error: e.message }); }
};
...
const updateAdminPermissions = async (req, res) => {
    try {
        ...
        if (permissions) {
            await db.query(`
                INSERT INTO admin_permissions (user_id, can_manage_products, can_manage_orders,
                    can_manage_users, can_manage_teachers, can_manage_students,
                    can_manage_classes, can_manage_finance, can_view_reports)
                VALUES (?,?,?,?,?,?,?,?,?)
                ...
                [user_id,
                 permissions.can_manage_products   ? 1 : 0,
                 permissions.can_manage_orders      ? 1 : 0,
                 permissions.can_manage_users       ? 1 : 0,
                 permissions.can_manage_teachers    ? 1 : 0,
                 permissions.can_manage_students    ? 1 : 0,
                 permissions.can_manage_classes     ? 1 : 0,
                 permissions.can_manage_finance     ? 1 : 0,
