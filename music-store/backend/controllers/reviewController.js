...
const createReview = async (req, res) => {
    try {
        ...
        const [purchased] = await db.query(`
            ...
            WHERE o.user_id=? AND od.product_id=? AND o.status='delivered'`, [user_id, product_id]);
        if (!purchased.length)
            return res.status(403).json({ message: 'Bạn cần mua và nhận sản phẩm trước khi đánh giá!' });
        const [existing] = await db.query(
            'SELECT review_id FROM reviews WHERE user_id=? AND product_id=?', [user_id, product_id]);
        if (existing.length) return res.status(400).json({ message: 'Bạn đã đánh giá sản phẩm này rồi!' });
        await db.query('INSERT INTO reviews (product_id,user_id,rating,comment) VALUES (?,?,?,?)',
...
const deleteReview = async (req, res) => {
    try {
        ...
        if (req.user.role !== 'admin' && req.user.role !== 'super_admin' && review.user_id !== req.user.user_id)
...
const getReviewsByProduct = async (req, res) => {
    try {
        const [reviews] = await db.query(`
            ...
            WHERE r.product_id=? ORDER BY r.created_at DESC`, [req.params.product_id]);
