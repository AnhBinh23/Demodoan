...
const getSlots = async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM work_slots WHERE is_active=1 ORDER BY slot_id');
        res.json(rows);
...
const getByDate = async (req, res) => {
    try {
        ...
        const [teachers] = await db.query(
            `SELECT t.teacher_id, t.full_name, t.avatar, t.specialty
             FROM teachers t WHERE t.is_active = 1 ORDER BY t.full_name`);
        const [slots] = await db.query('SELECT * FROM work_slots WHERE is_active=1 ORDER BY slot_id');
        const [records] = await db.query(
            `SELECT tk.teacher_id, tk.slot_id, tk.status, tk.note
             FROM teacher_timekeeping tk WHERE tk.work_date = ?`, [date]);
        const map = {};
...
const saveRecord = async (req, res) => {
    try {
        ...
        await db.query(
            `INSERT INTO teacher_timekeeping (teacher_id, work_date, slot_id, status, note, created_by)
             VALUES (?,?,?,?,?,?)
        ...
        for (const r of records) {
            await db.query(
                `INSERT INTO teacher_timekeeping (teacher_id, work_date, slot_id, status, note, created_by)
                 ...
                 ON DUPLICATE KEY UPDATE status=VALUES(status), note=VALUES(note),
                 created_by=VALUES(created_by), updated_at=NOW()`,
                [r.teacher_id, work_date, r.slot_id, r.status||'present', r.note||null, req.user.user_id]
            );
        }
    ...
    try {
        ...
        const [rows] = await db.query(
            `SELECT tk.work_date, tk.slot_id, tk.status, tk.note,
                    ws.label, ws.start_time, ws.end_time
             FROM teacher_timekeeping tk
             JOIN work_slots ws ON tk.slot_id = ws.slot_id
             WHERE tk.teacher_id = ? AND DATE_FORMAT(tk.work_date,'%Y-%m') = ?
             ORDER BY tk.work_date, ws.slot_id`,
            [teacher_id, month]
        );
    ...
    try {
        ...
        const [rows] = await db.query(
            `SELECT t.teacher_id, t.full_name, t.avatar, t.specialty,
                ...
                t.salary, t.salary_type
             FROM teachers t
             LEFT JOIN teacher_timekeeping tk ON t.teacher_id = tk.teacher_id
               AND DATE_FORMAT(tk.work_date,'%Y-%m') = ?
             WHERE t.is_active = 1
             GROUP BY t.teacher_id ORDER BY t.full_name`,
            [month]
        );
        res.json(rows);
    } catch(e) { res.status(500).json({ message: 'Lỗi server!', error: e.message }); }
